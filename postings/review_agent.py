...
- 급여 환산 기준:
  · 본문이 '세전(세금 전) 월급·연봉'을 제시한 경우에는 그 세전 월 금액(연봉이면 12로 나눈 값)을 net_salary_pretax 필드로 제안한다. 시스템이 파이프라인과 동일한 세금 공식으로 세후 월급(net_salary)으로 환산해 반영하므로, 세후 값을 직접 계산하지 않아도 된다.
