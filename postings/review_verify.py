...
- 세전/세후: '세전'·'세전계약'이면 세전, '실수령'·'세후'면 세후. 단서가 없으면 세후로 간주.
  ...
  · 세전→세후 환산 공식(시스템 파이프라인과 동일): 세후월급 = 5.35 + 0.904394 × 세전월급 − 0.000143950695 × 세전월급².
    (네이버 세금계산기 표본 회귀식이라 4대보험·소득세가 이미 반영돼 있다. 세전 공고의 net_salary 검산은 반드시 이 공식으로 하고,
     정률 공제(예: 9%·20%·3.3%)로 어림해 기대값을 만들지 말라. 실효 공제율은 소득이 클수록 커진다.)
  · 세후 시급(net_hourly_wage) = 세후월급 ÷ 월 근무시간. 세후월급이 위 공식과 3% 이내로 맞으면 그로부터 나온 시급도 정상이다.
  · 급여가 범위(예: '세전 650~700만')거나 복수 시나리오면, 저장값이 그중 한 끝값을 기준으로 계산됐으면 정답으로 인정하라.
    다른 끝값(상단↔하단)으로 재계산해 불일치라고 판정하지 말라.
