...
**완료**
- **세전→세후 환산 위임 + 검산** (`net_salary_pretax` 가상 필드): 본문이 세전 금액을 제시하면 모델은
  ...
  아니라 updates 의 인자로 처리(구조화 출력, 아래 주의 참고). 프롬프트에 회귀식을 명시해, 모델이
  저장된 net_salary 가 세전 금액의 환산값과 맞는지 직접 검산하고 어긋나면 교정 제안하게 했다
  (드리프트 적발). 비교 허용 오차(`_values_equal`)는 스케일 무관 **상대 3%** 로 통일한다 — 월급 400만원
  이면 ±12만원, 시급 3.5만원이면 ±0.1만원 수준으로 단위 크기에 비례해 자동 조정된다(고정 절대 오차가
  월급에선 과하게 빡빡하던 문제 해결). 3% 미만 차이(예: 1%대 레거시 환산 차)는 정상으로 본다.
  세금 공제 메커니즘을 환각하지 않도록 "환산은 이 회귀식으로 설명한다"고 못박았다.
