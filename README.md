...
7. [배포 (Cloud Run)](#배포-cloud-run)
8. [인증 (Supabase 로그인)](#인증-supabase-로그인)
9. [Notion 통계 생성](#notion-통계-생성)
10. [모듈별 설명](#모듈별-설명)
11. [LLM 파이프라인 상세](#llm-파이프라인-상세)
12. [데이터 모델](#데이터-모델)
13. [LLM 비용 모니터링](#llm-비용-모니터링)
14. [일회성 데이터 마이그레이션](#일회성-데이터-마이그레이션)
...
│   ├── settings.py          # 로컬(관리자) 기본 설정
│   ├── settings_prod.py     # 배포본(Cloud Run) 설정 — web 앱만, DB 없이 스냅샷 JSON, Supabase 게이트
│   └── urls.py              # /admin/ 은 admin 앱이 설치된 로컬에서만 노출
│
...
│   │   ├── export_snapshot.py # 최신 DashboardSnapshot을 web/snapshot/latest.json으로 export (배포 아티팩트)
...
│   ├── context_processors.py # supabase — 템플릿에 SUPABASE_URL/ANON_KEY 공개 설정 주입
│   ├── urls.py              # /login/ · /logout/ · /healthz · / · /compare/ · /compare/result/ · /method/ · /fulltime/ · /weekend/ · /etc/ · /onetime/
...
│       ├── auth.js          # Supabase 클라이언트: OAuth 콜백 처리·세션 쿠키 동기화·프로필 배너 채우기·로그아웃
│       └── ui.js            # 토글 등 인터랙션
│
...
│
├── deploy.sh                # 공개 배포본을 Cloud Run에 배포 (스냅샷 export → 시크릿 동기화 → gcloud run deploy)
├── Dockerfile              # 배포본 컨테이너 이미지 (web 앱 + gunicorn + whitenoise)
├── .dockerignore / .gcloudignore  # 배포 이미지에서 관리자 코드·로컬 데이터 제외
├── requirements-web.txt    # 배포본 전용 최소 의존성 (django/gunicorn/whitenoise/PyJWT, 크롤러·LLM 제외)
├── .env.prod.example       # 배포 설정 템플릿 (Supabase URL/키 등) → .env.prod로 복사 (gitignore)
│
└── requirements.txt         # 로컬(관리자) 전체 의존성
```

> `web/snapshot/latest.json`(배포 아티팩트)은 `deploy.sh` 실행 시 `export_snapshot`이 생성하며 `.gitignore` 대상이다.
...
로컬(관리자) 실행에 필요한 변수는 프로젝트 루트의 `.env` 파일에 설정한다. 공개 배포본(Cloud Run) 전용 변수(Supabase 로그인 등)는 별도의 `.env.prod`에 두며 `deploy.sh`가 읽는다 ([인증 (Supabase 로그인)](#인증-supabase-로그인) 참고).
...
2. **선택 항목 LLM 처리** 버튼을 누르면 먼저 처리 대상을 정하는 in-app 모달(`#count-modal`, LLM 자동 검토와 동일한 보라색 디자인)이 뜬다. 두 경로가 이 모달을 공통으로 거치며, 브라우저 기본 `prompt()`/`confirm()`은 쓰지 않는다 — 모달 자체가 API 비용 경고 배너와 함께 확인 역할을 한다.
   - **체크한 행이 있으면**: 모달이 "선택한 N건을 처리합니다" 확인만 받는다.
   - **체크가 없으면**: 페이지네이션과 무관하게 몇 건을 처리할지 입력받는다. 빠른 선택 칩(총 대기 수 미만인 25/50/100/250/500 값 + "전체")과 숫자 입력을 제공하며, 확정하면 `prestage/pending-ids/`가 현재 정렬 순서대로 앞 N건의 id·title을 반환한다(현재 페이지 25행에 국한되지 않음).
   확정하면 LLM 자동 검토와 동일한 **실시간 진행 모달**이 뜨고, 공고끼리는 독립적이므로 **동시성 6의 워커 풀**로 여러 건을 병렬 처리한다(건당 `prestage/process-one/` 호출 → `pipeline.stages.process_raw_posting`; 직렬 대비 대폭 단축, Gemini rate limit 고려한 상한). 중지 시 진행 중인 건은 끝내되 새 건은 시작하지 않는다. 각 건은 저장/급여 미명시 건너뜀/에러로 집계된다. 모달 행에는 추출된 핵심 필드(시급·월급·지역·근무형태·주당 시간, `pipeline.stages._summary_fields`)가 인라인으로 표시되고, 행을 클릭하면 5-task 처리 단계 요약(`process_posting`이 모은 `steps`)이 펼쳐진다. 완료 후 대기 목록과 배지가 자동 갱신된다.
...
| `jobposting/review/prestage/pending-ids/` | AJAX GET: 처리 대기 RawPosting을 정렬 순서대로 앞 `count`건의 id·title로 반환 (페이지네이션과 무관한 배치 대상 선정) |
...
| 급여 유형 | 표기 정규화(`montly`→`monthly` 등, 빈 값·`None`은 미명시로 간주해 무시) 후에도 알 수 없는 유형만 → `monthly`로 대체하며 에러 기록 | — |
| 세전/세후 | — | 본문에 "세전" 포함 |
...
1. 버튼을 누르면 먼저 검토 대상을 정하는 in-app 개수 모달이 뜬다 (pre-2단계 LLM 프로세싱과 동일한 `openCountModal`/`confirmRunModal`을 재사용하며, 브라우저 기본 `confirm()`은 쓰지 않는다). 표에서 체크한 행이 있으면 "선택한 N건을 검토합니다" 확인만 받고, 체크가 없으면 후보를 조회한 뒤 위에서부터 몇 건을 검토할지(빠른 선택 칩 + 숫자 입력) 고른다. 확정하면 실시간 진행 모달이 뜬다.
...
4. 이미 처리된 공고(`has_error=True` 또는 `AdminCheck` 존재)는 건너뛴다. 공고끼리 독립적이므로 진행 모달은 **동시성 6의 워커 풀**로 여러 건을 병렬 검토한다(pre-2단계 LLM 프로세싱과 동일한 방식, 직렬 대비 대폭 단축). 중지 시 진행 중인 건은 끝내되 새 건은 시작하지 않는다. 모달은 항목별 로그(제목 + 사유)와 함께 건별 토큰/USD 비용(`pipeline/pricing.py`의 `compute_cost`), 그리고 배치 전체 누적 토큰·비용을 표시하며 중지/닫기를 지원한다.
...
**왜 function calling이 아니라 구조화 출력인가:** Gemini 2.5는 `thinking + function calling` 조합에서 `MALFORMED_FUNCTION_CALL`을 빈번히(측정 ~12%) 낸다. 응답을 `response_schema`(제약 디코딩, `{message, updates}`)로 강제하면 형식 오류가 구조적으로 불가능해, thinking을 켠 채로도 실패하지 않는다. 검토 완료 코멘트 생성(`generate_comment`)은 **결정론 기본 + LLM 보조** 방식이다 — 사용자가 아무것도 논의하지 않은 자명한 케이스(자동 브리핑만 있고 사용자 발화·거부 지시가 없음, `_has_user_discussion`)는 LLM 없이 적용된 변경 목록(또는 "수정 없음")으로 결정론 코멘트를 쓰고, 실제 다턴 논의가 있을 때만 LLM 요약을 시도한다. LLM 경로는 반대로 thinking을 끄고(`thinking_budget=0`) 토큰 한도를 높여 코멘트가 잘리지 않게 하며, transcript 를 `<transcript>…</transcript>`로 감싸 "이어쓸 대화"가 아니라 "요약 대상"임을 못박는다. 출력은 `_looks_like_summary`로 검증해 요약답지 않으면(역할 태그 재등장·과도한 길이 등 이어쓰기·환각 신호) 결정론 코멘트로 폴백한다 — 모델이 관련 없는 가짜 대화를 지어내던 환각을 차단한다.
...
| URL | 페이지 | 내용 |
|---|---|---|
| `/` | 홈 | 분석 공고 수, 근무 유형(전국·풀타임·주말 파트·기타 파트·일회성)별 평균 시급, 지역 5분류별 평균 시급, **연간 시급 상승률** 모달(전국 장기 근무 전체 공고의 등록일별 시급 산점도 + 추세선) |
...
| `/fulltime/` | 풀타임 | 주당 36시간 이상 지속성 근무. 근무시간 히스토그램·시급/월급 산점도(회귀선)·등록일별 산점도·지역별 평균/분포(바이올린), 퇴직금 보정(시급 ×13/12) 토글 |
| `/weekend/` | 주말 파트 | 평일 근무 0 & 주말 근무가 있는 공고. 히스토그램·등록일별 산점도·지역별 평균·분포(바이올린) |
| `/etc/` | 기타 파트 | 36시간 미만 지속성 근무 중 주말 파트가 아닌 공고. 히스토그램·산점도(회귀선)·등록일별 산점도·지역별 평균·분포(바이올린) |
...
DashboardSnapshot 갱신 (리뷰 대시보드 "대시보드 업데이트" 버튼)
    ↓
postings/dataframe.build_dataframe(exclude_pending_review=True)  # 미검토 문제 큐(2·3단계) 공고 제외 → 통계 호환 DataFrame
    ↓
...
DashboardSnapshot(data=…, posting_count=…) 저장 (created_at = "Last Update")
    ↓
web/snapshot.get_latest_snapshot(): 로컬은 DB, 배포본은 JSON 파일에서 읽음
    ↓
web/views: 스냅샷의 해당 섹션을 json_script로 템플릿에 임베드
    ↓
...
- **차트 렌더링**: `web/static/web/js/charts.js`는 데이터 입력과 분리된 순수 SVG 빌더(`buildHistogram`, `buildScatter`, `buildBubble`, 등록일별 산점도용 `buildDateScatter`, 시급 분포용 `buildViolin`, "내 시급 비교"용 `buildCompare`/`buildHoursScatter` 등)로, 입력 형태만 맞으면 실데이터/목업 어느 쪽이든 동일하게 렌더한다. 페이지별 `*.js`가 임베드 데이터를 각 빌더에 바인딩한다. 홈의 **연간 시급 상승률** 모달과 근무 유형별 4개 페이지(풀타임/주말/기타/일회성)의 등록일별 시급 산점도는 `buildDateScatter`로 렌더하며(홈은 전국 장기 근무 전체 공고를 한 그래프에 모아 표시), 최소제곱 회귀 **추세선**(빨강)과 그 끝에 연간 시급 변화량을 흰 배경 뱃지(예: `+1,079원/년`)로 표기한다. `yearLines` 옵션을 켜면 매년 1월 1일 경계에 옅은 점선 세로선과 연도 라벨(`○○년`)을 그려 연도 구분을 표시한다. 근무 유형별 페이지의 지역 시급 분포는 바이올린 차트(`buildViolin`: 폭=시급대 공고 밀도, 흰 박스=IQR·중앙값, 빨간 사각형=평균)로 렌더한다.
...
배포본은 `DJANGO_SETTINGS_MODULE=config.settings_prod`로 뜨며, `config/settings_prod.py`가 로컬 설정을 다음과 같이 덮어쓴다:

- **DB 없음**: `INSTALLED_APPS`에서 `postings`·`admin` 등 관리자 앱을 빼고 `web`(+`staticfiles`)만 남긴다. 따라서 pandas·selenium·google-genai 같은 무거운 의존성이 필요 없고, SQLite DB에도 접근하지 않는다. `/admin/` 라우트는 `config/urls.py`가 `admin` 앱 설치 여부를 보고 로컬에서만 노출한다.
- **스냅샷 JSON에서 데이터 로드**: `SNAPSHOT_FROM_FILE=True`로 두어, 공개 페이지가 DB 대신 번들된 `web/snapshot/latest.json`을 읽는다 ([데이터 파이프라인 (스냅샷 방식)](#데이터-파이프라인-스냅샷-방식) 참고).
- **정적 파일**: `whitenoise`가 압축·해시(캐시버스팅) 방식으로 직접 서빙한다 (`collectstatic`은 이미지 빌드 시 수행).
- **Supabase 로그인 + 초대코드 게이트**: `SupabaseAuthMiddleware`가 활성화된다 ([인증 (Supabase 로그인)](#인증-supabase-로그인) 참고).

### 배포본 전용 파일

| 파일 | 역할 |
|---|---|
| `config/settings_prod.py` | 배포 설정 (위 항목). `ALLOWED_HOSTS`·`AUTH_GATE`·`INVITE_GATE` 등은 env로 주입 |
| `requirements-web.txt` | 배포 전용 최소 의존성 (`django`/`gunicorn`/`whitenoise`/`python-dotenv`/`PyJWT[crypto]`) |
| `Dockerfile` | `python:3.13-slim` 위에 `requirements-web.txt` 설치 → `collectstatic` → gunicorn 실행 (`$PORT`, 기본 8080) |
| `.dockerignore` / `.gcloudignore` | 관리자 코드(`postings`/`scraper`/`pipeline`/`geo`/`notion-stats`/…)·로컬 DB·시크릿을 이미지에서 제외. `web/snapshot/latest.json`은 아티팩트라 **제외하지 않는다** |
| `deploy.sh` | 원클릭 배포 스크립트 (아래) |
| `postings/management/commands/export_snapshot.py` | 최신 `DashboardSnapshot`을 `web/snapshot/latest.json`으로 export (배포 아티팩트 생성) |
| `.env.prod.example` | 배포 설정 템플릿 → `.env.prod`(gitignore)로 복사해 채운다 |
...
`deploy.sh`가 다음을 순서대로 수행한다:

1. **스냅샷 export**: `python manage.py export_snapshot`으로 로컬 DB 최신 스냅샷을 `web/snapshot/latest.json`으로 내보낸다. (먼저 어드민에서 **대시보드 업데이트**로 스냅샷을 만들어 두어야 한다.)
2. **시크릿 동기화**: `.env.prod`의 값을 Google Secret Manager(`gongo-` 네임스페이스)에 생성/갱신하고 Cloud Run SA에 접근 권한을 부여한다. `DJANGO_SECRET_KEY`는 없으면 자동 생성하고, `SUPABASE_JWT_SECRET`은 있을 때만 등록한다.
3. **공개 env 주입**: `ALLOWED_HOSTS`·`AUTH_GATE`·`SUPABASE_URL`·`SUPABASE_ANON_KEY`를 env로 넘긴다.
4. **`gcloud run deploy --source .`**: Cloud Build가 `Dockerfile`로 이미지를 빌드해 배포한다.

실제 비밀은 `.env.prod`에서만 읽으므로 스크립트에 비밀이 박히지 않는다. 선행 1회 작업(GCP 프로젝트 설정, 필요한 API 활성화, `.env.prod` 작성, 커스텀 도메인 매핑, Supabase Google provider 설정)은 `deploy.sh` 상단 주석에 정리돼 있다.

---

## 인증 (Supabase 로그인)

공개 배포본에는 **두 계층의 진입 게이트**가 있다 (베타 기간 접근 제한용): ① Supabase Google **로그인**(JWT), ② 로그인 뒤 **초대코드**(allow-list). 로컬 개발에서는 `SUPABASE_URL`이 비어 있어 게이트와 인증 스크립트가 모두 비활성 상태다.
...
초대코드 게이트에 필요한 테이블·RPC는 SQL 마이그레이션으로 정의되며, Supabase **SQL Editor에서 순서대로 수동 실행**한다(Django ORM과 무관, 배포 파이프라인에 포함되지 않음).
...
`web/context_processors.py`의 `supabase` 컨텍스트 프로세서가 `SUPABASE_URL`·`SUPABASE_ANON_KEY`를 템플릿에 노출한다 (anon/publishable 키는 브라우저 공개용이라 노출 안전).
...
| `tasks.py` | `run_task_1()~run_task_5()` — Gemini API 호출 후 JSON 파싱, `extract_json()` 포함 (`raw_decode`로 첫 번째 유효 JSON만 추출). 추출 태스크는 few-shot 기반 구조화 JSON 추출이라 사고(thinking)가 사실상 불필요하므로 `_gen_config()`가 `thinking_budget=0`으로 thinking을 꺼 지연·비용을 줄인다(단, 최소 사고 예산이 강제되는 `pro` 계열은 예외로 기본 설정 사용) |
...
| `stages.py` | 2단계 파이프라인 공통 로직. `scrape_stage()` — 크롤링하여 `RawPosting(pending)` 저장. `process_stage()` — pending RawPosting을 일괄 LLM 처리. `process_raw_posting(raw, client, ..., capture_stdout=True)` — 단일 RawPosting 처리 단위 함수(일괄 처리와 대시보드 건별 처리가 공유). 대시보드 모달용으로 `process_posting`의 `steps`와 핵심 추출 필드 요약(`_summary_fields`: 시급·월급·지역·근무형태·주당 시간)을 반환 dict에 실어 보낸다. `capture_stdout`는 tasks/validator의 `print()`를 전역 `sys.stdout` 스왑으로 가로채 log로 흘리는데, 이 스왑은 프로세스 전역이라 동시 호출 시 서로를 덮어쓰므로 병렬 경로(대시보드 동시성 6 `process-one` 등)에서는 `capture_stdout=False`로 꺼 동시성 안전을 확보한다. 모두 멱등하다 |
| `validator.py` | `error_check(d, error_history)` — 시급·근무 시간 일관성 검증, 주당 근무 시간 재계산. `normalize_wage_type(value)` — 급여 유형 표기 흔들림(`montly` 오타 등)을 표준형으로 흡수하고 빈 값·`None`은 미명시로 보아 `None` 반환(오검출 방지) |
...
| 파일 | 역할 |
|---|---|
| `dataframe.py` | `build_dataframe(exclude_pending_review=False)` — Django DB의 `JobPosting`을 통계 스크립트 호환 형식(한글 컬럼명, boolean → `Yes`/`No`)의 pandas DataFrame으로 변환. `run_statistics` 커맨드와 `dashboard_stats`가 함께 사용. `exclude_pending_review=True`(대시보드 스냅샷 생성 경로)면 미검토 상태로 2·3단계 문제 큐에 걸린 공고를 `review_presets.pending_review_pks`로 골라 제외 |
