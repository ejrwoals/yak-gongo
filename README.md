...
- **차트 렌더링**: `web/static/web/js/charts.js`는 데이터 입력과 분리된 순수 SVG 빌더(`buildHistogram`, `buildScatter`, `buildBubble`, 등록일별 산점도용 `buildDateScatter`, 시급 분포용 `buildViolin`, "내 시급 비교"용 `buildCompare`/`buildHoursScatter` 등)로, 입력 형태만 맞으면 실데이터/목업 어느 쪽이든 동일하게 렌더한다. 페이지별 `*.js`가 임베드 데이터를 각 빌더에 바인딩한다. 근무 유형별 4개 페이지(풀타임/주말/기타/일회성)의 등록일별 시급 산점도는 `buildDateScatter`로 렌더하며, 최소제곱 회귀 **추세선**(빨강)과 그 끝에 연간 시급 변화량을 흰 배경 뱃지(예: `+1,079원/년`)로 표기한다. 근무 유형별 페이지의 지역 시급 분포는 바이올린 차트(`buildViolin`: 폭=시급대 공고 밀도, 흰 박스=IQR·중앙값, 빨간 사각형=평균)로 렌더한다.
